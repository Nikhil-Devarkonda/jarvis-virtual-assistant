...
def remember(param):
    ...
    if len(token) >= 2 : 
        ...
        print(param['TEXT'])
        f = open('Data/remembers.list')
        list_ = ast.literal_eval(f.read())
        list_.insert(0,param['TEXT'])
        f.close()
        f = open('Data/remembers.list','w')
        f.write(str(list_))
        f.close()
    else:
...
class Extra:
    ...
    def tell_news(param={}):
        try:
            ...
            print("Imported:request,json,webbrowser")
    ...
    def add_event(param={}):
        try:
            ...
            # Key ['Jan 6']
            key = month+" "+str(day)
            f = open("Data/calender_events.json")
            ...
            f = open("Data/calender_events.json",'w')
            f.write(events_j)
            f.close()
            # print(events_j)
        except Exception as e:
